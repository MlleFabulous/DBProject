...
//		
	...
	private void printSchedule(ArrayList<Schedule> schedule) {
		dynamicSchedulePanel.removeAll();
		for(int i=0; i<schedule.size();i++) {
			dynamicSchedulePanel.add(new JLabel(schedule.get(i).getScene()));
			dynamicSchedulePanel.add(new JLabel(schedule.get(i).getBand()));
			dynamicSchedulePanel.add(new JLabel(schedule.get(i).getStart()));
			dynamicSchedulePanel.add(new JLabel(schedule.get(i).getEnd()));
		}
		frame.revalidate();
	}
...
//		middlepanel.add(listScenes, BorderLayout.CENTER);
	...
	public JPanel areaBottomPanel() {
		...
		int newRows = 1;
		dynamicSchedulePanel = new JPanel(new GridLayout(0, 4));
