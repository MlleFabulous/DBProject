...
public class BookBandUI extends JPanel implements ActionListener {
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource()==btnBook) {
			...
			String workerPersonNbr = strWorker.split("--")[1];
			if((strWorker!="<select contact person>")&&(!tfBand.getText().equals("<band name>"))&&
					(!tfCountry.getText().equals("<band country>"))&&
					(!tfInfo.getText().equals("<band info>"))&&
					(!taArtists.getText().equals("<name, role ( seperate multiple artists in band by '.' )>"))) {
				
				controller.insertBand(tfBand.getText(), tfCountry.getText(), tfInfo.getText(), workerPersonNbr);

				//then create the entity Artists in DB
				String artistsUncutted = taArtists.getText();
				System.out.println(artistsUncutted);
